...
*/
#define VERIFY( name, x ) { if (!(x)) cart_error( "Invalid value for parameter %s: constraint %s is violated (file %s line %u)", #name, #x, __FILE__, __LINE__ ); control_parameter_set_verified(#name); }
