...
halo_list *load_halo_finder_catalog( char *filename ) {
	...
	cart_debug("loading %u halos", halos->num_halos );
	...
	while ( fscanf( input, "%u %e %e %e %e %e %e %e %e %e %u %e %e %e %u",
		...
		halos->list[h].particles = NULL;
		halos->list[h].binding_order = NULL;
	...
	cart_debug("Done loading halo list");
	...
	if ( local_proc_id == MASTER_NODE ) {
		for ( ihalo = 0; ihalo < num_halos; ihalo++ ) {
			...
			fprintf(output,"%u %e %e %e %e %e\n", halos->list[ihalo].id, 
				total_stellar_mass[ihalo], total_dm_mass[ihalo], halos->list[ihalo].mvir, 
...
void compute_halo_properties( char *analysis_directory, int halo_section, halo_list *halos ) {
	...
	for ( i = 0; i < num_radii; i++ ) {
		if ( radii_units[i] == 'c' ) {
			radii_overdensity[i] = radii_delta[i] / Omega0;
		} else {
		...
		if ( halo_section == -1 || halos->list[ihalo].section == halo_section ) {
			...
			if ( found ) {
				...
				for ( leaf = 0; leaf < num_halo_leafs; leaf++ ) {
					...
					Scell = Sfact * cell_gas_internal_energy(icell)*pow(rhogi,gamma);
...
*/
					...
					if ( min_bin < num_bins ) {
						...
						if ( max_bin == min_bin ) {
							...
							bin_gas_metallicity_II[min_bin] += cell_gas_metallicity_II(icell)*rhogi*cell_mass;
							bin_gas_metallicity_Ia[min_bin] += cell_gas_metallicity_Ia(icell)*rhogi*cell_mass;
...
#ifdef ADVECT_SPECIES
									bin_gas_metallicity_II[bin] += cell_gas_metallicity_II(icell) *
										rhogi * mass_fraction; 
									bin_gas_metallicity_Ia[bin] += cell_gas_metallicity_Ia(icell) *
										rhogi * mass_fraction;
...
#endif /* COOLING */
#ifdef ADVECT_SPECIES
						bin_gas_metallicity_II[bin] += cell_gas_metallicity_II(icell)*rhogi*cell_mass;
						bin_gas_metallicity_Ia[bin] += cell_gas_metallicity_Ia(icell)*rhogi*cell_mass;
#endif /* ADVECT_SPECIES */
				...
				if ( halos->list[ihalo].proc == local_proc_id ) {
					...
					for ( bin = 0; bin < num_bins; bin++ ) {
						fprintf( bgpro, "%.3f %.3f %e %e %e %e %e %.3f %.3f %e %e\n",
								...
								bin_gas_tcool[bin],
								bin_gas_metallicity_II[bin]/Zsolar,
								bin_gas_metallicity_Ia[bin]/Zsolar
						       );
					}
					...
					for ( bin = 0; bin < num_bins; bin++ ) {
						if ( bin_star_age[bin] > 0 ) {
							age_star = age_t( bin_star_age[bin] );
						} else {
							age_star = -1.0;
						}
						...
						fprintf( bzpro, "%.3f %.3f %e %e %e %e %e %e %.3f\n",
								rmid[bin]*rfact*hubble, rr[bin]*rfact*hubble,
								bin_gas_metallicity_II[bin]/Zsolar, bin_gas_metallicity_Ia[bin]/Zsolar,
								bin_star_metallicity_II[bin]/Zsolar, bin_star_metallicity_Ia[bin]/Zsolar,
								bin_new_star_metallicity_II[bin]/Zsolar, 
								bin_new_star_metallicity_Ia[bin]/Zsolar,
								age_star );
					}
				...
				for ( leaf = 0; leaf < num_halo_leafs; leaf++ ) {
					...
					r = compute_distance_periodic_float( halos->list[ihalo].pos, cell_pos ) / rvir;
				...
				if ( halos->list[ihalo].proc == local_proc_id ) {
					...
					fprintf( bmprovir, "# %u %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %.3f\n",
							...
							halos->list[ihalo].mvir, vmax, rmax, avg_gas_metallicity_II, 
					...
					for ( bin = 0; bin < num_vir_bins; bin++ ) {
						...
						fprintf( bzprovir, "%.3f %.3f %e %e %e %e %e %e %.3f\n",
								...
								bin_gas_metallicity_II[bin]/Zsolar, 
								bin_gas_metallicity_Ia[bin]/Zsolar,
								bin_star_metallicity_II[bin]/Zsolar, 
								bin_star_metallicity_Ia[bin]/Zsolar,
								bin_new_star_metallicity_II[bin]/Zsolar, 
								bin_new_star_metallicity_Ia[bin]/Zsolar,
								age_star );
					}
