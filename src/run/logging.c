...
void log_star_creation( int icell, double mass, int fileop_temp ){
  ...
  switch(fileop_temp)
    ...
    case FILE_OPEN:
      {
	sprintf(filename,"%s/star_temp.%05u.log",output_directory,local_proc_id);
...
void combine_star_creation_log(){
  ...
  for ( proc = 0; proc < num_procs; proc++ ) {
    sprintf(filename_local,"%s/star_temp.%05u.log",output_directory,proc);
...
void init_logging( int restart ) {
	...
	sprintf(filename, "%s/timing.%05u.log", logfile_directory, local_proc_id );
...
#ifdef PAPI_PROFILING                                                                                                                                                                 
	sprintf( filename, "%s/papi_profile_%s.%05u.log", logfile_directory, papi_eventset_description, local_proc_id );
	...
	sprintf(filename, "%s/workload.%05u.dat", logfile_directory, local_proc_id );
	...
	sprintf(filename, "%s/dependency.%05u.dat", logfile_directory, local_proc_id );
