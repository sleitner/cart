...
void read_indexed_particles( char *header_filename, char *data_filename, char *stellar_filename,
	...
	fseek( input, (long)num_grid*(long)num_grid*(long)num_grid*sizeof(long), SEEK_SET );
	...
	/* skip over index */
	fseek( stellar_input, (long)num_grid*(long)num_grid*(long)num_grid*sizeof(long), SEEK_SET );
#endif /* STARFORM */
	...
	for ( sfc = 0; sfc < max_sfc_index; sfc++ ) {
...
#ifdef HYDRO
void read_indexed_grid( char *filename, int max_level_to_read, void callback( cell_struct * ) ) {
	int i, j, k, m, n;
	FILE *input;
	int size;
	int endian;
	...
	cart_debug("ncell0 = %u", ncell0 );
	cart_debug("minlevel = %u", minlevel );
	cart_debug("maxlevel = %u", maxlevel );

	init_sfc();

	fseek( input, 2*sizeof(int)+(long)ncell0*sizeof(long), SEEK_CUR );

	for ( sfc = 0; sfc < ncell0; sfc++ ) {
		fread( &size, sizeof(int), 1, input );
		if ( endian ) {
			reorder( (char *)&size, sizeof(int) );
		...
		for ( level = 0; level <= maxlevel; level++ ) {
			total_cells += (long)cells_per_level[level];
		}

		cell_refined_to_read = total_cells-cells_per_level[maxlevel];

		cell_refined = cart_alloc( (long)cell_refined_to_read*sizeof(int) );
		vars = cart_alloc( (long)num_file_vars*(long)total_cells*sizeof(float) );
		...
		pos_level = cart_alloc( 3*sizeof(float) );
		...
		for ( level = minlevel; level <= min( maxlevel, max_level_to_read ); level++ ) {
			if ( level < min( maxlevel, max_level_to_read ) ) {
				pos_next_level = cart_alloc( 3*cells_per_level[level+1]*sizeof(float) );
				next_level_count = 0;
			} else {
				pos_next_level = NULL;
			}
			...
			for ( j = 0; j < cells_per_level[level]; j++ ) {
				cell.level = level;
				cell.is_leaf = !cell_refined[icell+j];

				for ( k = 0; k < nDim; k++ ) {
					cell.pos[k] = pos_level[3*j+k];
				}
				...
				if ( level < min( maxlevel, max_level_to_read ) && cell_refined[icell+j] ) {
					for ( k = 0; k < 8; k++ ) {
						for ( dim = 0; dim < 3; dim++ ) {
							pos_next_level[3*next_level_count+3*k+dim] =
								pos_level[3*j+dim] + 0.5*cell_size[level]*cell_delta[k][dim];
						}
					}
