...
int main ( int argc, char *argv[]) {
	...
	char phase_output_directory[256];
	...
	if ( argc != 10 ) {
		cart_error("Usage: ./cart_halo_phase_diagram Lbox jobname output_directory halofinder_directory profiles_directory analysis_directory rvir sample_file aexpn");
	}
	...
	strcpy( phase_output_directory, argv[6] );
	...
	for ( i = 0; i < halos->num_halos; i++ ) {
		halos->list[i].rvir = 20./r0;
	}
	*/
        cart_debug("done loading halo virial radii");

        /* compute halo properties */
        compute_halo_properties( output_directory, phase_output_directory, halos );
