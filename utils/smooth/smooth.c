...
int smInit(SMX *psmx,KD kd,int nSmooth,float *fPeriod)
{
	...
	for (j=0;j<3;++j) {
		if (root->bnd.fMax[j] - root->bnd.fMin[j] > fPeriod[j]) {
			...
			fprintf(stderr,"min, max, period = %f %f %f.\n", root->bnd.fMin[j],root->bnd.fMax[j],fPeriod[j]);
			bError = 1;
			}
		}
	if (bError) exit(1);
...
void smOutDensityBin(SMX smx,FILE *fp)
{
	int i,iCnt,nCnt,ntemp;
	float *myden;
	...
	fwrite(&ntemp,4,1,fp);
	nCnt = smx->kd->nDark; printf("nDark =%d\n",nCnt);
	if ( (nwritten = fwrite(&nCnt,sizeof(int),1,fp))!=1 )
	    {
	      printf("cannot write number of particles in density file\n");
	      exit(1);
	      }
        fwrite(&ntemp,4,1,fp);

        myden = malloc(sizeof(float)*smx->kd->nDark);

	iCnt = 0;
	for (i=0;i<smx->kd->nDark;++i) {
		if (smx->kd->bDark) {
			if (smx->kd->p[iCnt].iMark)
			  myden[i]=smx->kd->p[iCnt].fDensity;
		        else myden[i]=0.0;
		        ++iCnt;
	        }
		else myden[i]=0.0;
	...
	ntemp = nCnt*4;
	fwrite(&ntemp,4,1,fp);
 	fwrite(myden,sizeof(float),nCnt,fp);
	fwrite(&ntemp,4,1,fp);
