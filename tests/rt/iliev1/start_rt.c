...
void FindIFront(float *riAvg, float *riMin, float *riMax)
{
  const long nside = 16;
...
void run_output()
{
  ...
  int varid[] = { RTU_FRACTION+RT_HVAR_OFFSET+0, HVAR_GAS_DENSITY, RTU_GAS_TEMPERATURE, RTU_CELL_LEVEL, RTU_LOCAL_PROC, RT_VAR_OT_FIELD };
  int nbin[] = { nbin1, nbin1, nbin1 };
  double bb[6];
  int done;
  float riAvg, riMin, riMax;
  double tPhys;
  char filename[99];
  FILE *f;
  ...
  if(local_proc_id == MASTER_NODE)
      ...
      else
	{
      ...
      cart_assert(f != 0);
      fprintf(f,"%6.2f %6.3f %6.3f %6.3f\n",(float)tPhys,riAvg,riMin,riMax);
      fclose(f);
...
void init_run()
{
   ...
   rtSingleSourceEps = 0.5*pow(0.5,(double)BottomLevel);
